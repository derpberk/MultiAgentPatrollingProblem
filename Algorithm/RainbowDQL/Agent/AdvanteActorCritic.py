...
from Algorithm.RainbowDQL.Networks.actorscritic import ActorCriticNetwork
...
class A2CAgent:
	...
	def sample_rollout(self):
		...
		while not all(dones.values()):
			...
			batch_actions, batch_log_probs, batch_values, batch_entropies = self.network.get_action(batch_states)
	...
	def train(self, episodes):
		...
		for episode in trange(1, int(episodes) + 1):
			...
			mean_reward = np.mean(total_reward_list[-50:])
			if mean_reward > record and len(total_reward_list) > 50:
				record = mean_reward
